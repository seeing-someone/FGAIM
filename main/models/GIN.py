...
class GIN(nn.Module):
    def __init__(self,
        ...
        if num_layers == 1:
            apply_func = MLP(1, in_dim, num_hidden, out_dim, norm=norm)
            if last_norm:
                apply_func = ApplyNodeFunc(apply_func, activation=activation)
            self.layers.append(
            ...
            self.layers.append(GINConv(
                in_dim,
                num_hidden,
                ApplyNodeFunc(MLP(1, in_dim, num_hidden, num_hidden, norm=norm),
            ...
            for l in range(1, num_layers - 1):
                ...
                self.layers.append(GINConv(
                    ...
                    ApplyNodeFunc(MLP(1, num_hidden, num_hidden, num_hidden, norm=norm),
                                  activation=activation),
                    init_eps=0,
                    learn_eps=learn_eps,
                    residual=residual)
            ...
            # output projection
            apply_func = MLP(1, num_hidden, num_hidden, out_dim, activation=activation, norm=norm)
            if last_norm:
                apply_func = ApplyNodeFunc(apply_func, activation=activation)
...
class ApplyNodeFunc(nn.Module):
    ...
    def __init__(self, mlp, activation="relu"):
